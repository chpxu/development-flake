  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      devshells.python = {extraModulesPath, ...}: let
        ...
        finalPythonPackages = (import ./packages.nix {inherit config pythonPackages lib;}).genOptionals;
      ...
      in {
        ...
        packages =
          lib.optionals cfg.uv.enable evaluateUV.packages
          ++ lib.optionals (!cfg.uv.enable) [(python.withPackages (_: finalPythonPackages))]
          ++ cfg.nixPackages;
